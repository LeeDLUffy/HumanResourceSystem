...
<?php
	...
	if(isset($_GET['search']))
	{
		...
		$RecordeLimit = 10;
		$search = mysqli_query($db,"select count(EmpId) as total from employee where RoleId !='1' and (Surname like '%".$SearchName."%' or Othernames like '%".$SearchName."%' or EmployeeId like '%".$SearchName."%')");
		...
		if(isset($_GET['bn']) &&intval($_GET['bn']) <= $number_of_row && intval($_GET['bn'] !=0))
		{
			...
			$sql = mysqli_query($db,"select * from employee where RoleId !='1' and (Surname like '%".$SearchName."%' or Othernames like '%".$SearchName."%' or EmployeeId like '%".$SearchName."%') LIMIT $Skip,$RecordeLimit ");
		}
		else
		{
			$sql = mysqli_query($db,"select * from employee where RoleId !='1' and (Surname like '%".$SearchName."%' or Othernames like '%".$SearchName."%' or EmployeeId like '%".$SearchName."%') LIMIT $RecordeLimit");
 	...
 	<div>
		...
		<thead>
			<tr>
				...
				<th style="text-transform: capitalize; text-align: center;">Full Details</th>
			</tr>
		</thead>
		<tbody>
		<?php while($row = mysqli_fetch_assoc($sql)) { ?>
			<tr>
				<td style="width: 140px;"><img id="myImg" alt="<?php $sname= (isset($row['Surname']))?$row['Surname']:""; $oname=(isset($row['Othernames']))?$row['Othernames']:"";
					echo ucfirst($sname)."&nbsp;".$oname; ?>" src="image/<?php echo $row['ImageName']; ?>" style="width: 50px;height: 50px;object-fit: cover;border-radius: 100%;border: 1px solid"></td>
				
				<td><?php $sname= (isset($row['Surname']))?$row['Surname']:""; $oname=(isset($row['Othernames']))?$row['Othernames']:"";echo ucfirst($sname)."&nbsp;".$oname; ?></td>
