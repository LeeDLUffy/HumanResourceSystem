...
<?php
  ...
  $laccept = mysqli_query($db,"SELECT l.*,e.Surname,e.Othernames,lt.type FROM leavedetails l JOIN employee e ON l.EmpId=e.EmployeeId JOIN leavetype lt on l.LeavetypeId=lt.LeaveId WHERE LeaveStatus='Accept'");
  $ldenied = mysqli_query($db,"SELECT l.*,e.Surname,e.Othernames,lt.type FROM leavedetails l JOIN employee e ON l.EmpId=e.EmployeeId JOIN leavetype lt on l.LeavetypeId=lt.LeaveId WHERE LeaveStatus='Denied'");
  
?>
<ol class="breadcrumb" style="margin: 10px 0px ! important;">
    <li class="breadcrumb-item"><a href="Home.php">Home</a><i class="fa fa-angle-right"></i>Leave<i class="fa fa-angle-right"></i>Leave Requests</li>
</ol>
<form method="POST">
<div class="validation-form">
  ...
  <div class="row" style="color: white; margin-right: 0; margin-left: 0;">
    ...
    <?php $i=1; while($row = mysqli_fetch_assoc($leavedetails)) {
      ...
      $namem = ucfirst($empname['Surname'])."&nbsp;".ucfirst($empname['Othernames']);
      $typen = $row['LeavetypeId'];
      $typeid = mysqli_query($db,"select * from leavetype where LeaveId='$typen'");
    ...
    <div class="row" style="margin-right: 0; margin-top: 10px; margin-left: 0;">
      ...
      <div class="col-md-2"><?php echo ucfirst((isset($typename['type']))?$typename['type']:"");?></div>
      <div class="col-md-1" ><?php echo ucfirst((isset($row['Reason']))?$row['Reason']:"");?></div>
      <div class="col-md-2" style="text-align: center;"><?php echo (isset($row['StartDate']))?$row['StartDate']:"";?></div>
...
<div class="row" style="color: white; margin-right: 0; margin-left: 0;">
  ...
  <div class="col-md-2" style="background-color: #202a29 ;">
    ...
    <?php $i=1; while($row = mysqli_fetch_assoc($laccept)) { 
      $name = ucfirst($row['Surname']." ".$row['Othernames']);
      ?>
<div class="row" style="margin-right: 0; margin-left: 0;">
  ...
  <div class="col-md-3">
    <h5><?php echo(isset($row['type']))?$row['type']:"";?></h5>
  </div>
  <div class="col-md-2">
    <h5><?php echo(isset($row['StartDate']))?$row['StartDate']:"";?></h5>
  </div>
  <div class="col-md-2">
