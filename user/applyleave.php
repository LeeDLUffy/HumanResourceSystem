...
<?php
  ...
  $sql = mysqli_query($db,"select * from leavetype ");
...
?>
               ...
               <div class="s-12 l-6">
                   ...
                   <div class="container-fluid" style="margin-bottom: 30px;margin-top: 10px; background: white;">
					    ...
					    <select id="country" name="leavestatus" title="Select Type of Leave" required="">
					    	<option value="">-- Select Type --</option>
                <?php while($row = mysqli_fetch_assoc($sql)) { ?>
                  <option value="<?php echo $row['LeaveId']; ?>"><?php echo ucfirst($row['type']);?></option>
                <?php } ?>
					    </select>
					    <label for="lname">Reason</label>
					    <input type="text" id="lname" name="reason" placeholder="Reason for application" title="Reason" required="" autocomplete="off">
